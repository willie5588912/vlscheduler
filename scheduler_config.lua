--[[
  VLC Scheduler Configuration Extension

  Provides a GUI for configuring the VLC Scheduler plugin.
  Set per-day schedules with times and media files.

  Install to VLC's lua/extensions directory, then find it
  under VLC > Extensions in the menu bar.
]]--
...
function descriptor()
    return {
        ...
        version = "1.2",
        author = "vlc-scheduler",
        shortdesc = "Scheduler Configuration",
        description = "Configure scheduled playlist playback for the "
                   .. "VLC Scheduler plugin. Set per-day schedules with "
                   .. "times and media files.",
        capabilities = {}
    }
end
...
function create_dialog()
    dlg = vlc.dialog("Scheduler Configuration")

    -- Row 1: Header
    dlg:add_label("<h3>VLC Scheduler Configuration</h3>", 1, 1, 6, 1)
...
function click_save()
    ...
    if count > 0 then
        ...
        if f then
            f:write("# VLC Scheduler Configuration\n")
            f:write("# Generated by Scheduler Config extension\n")
            f:write("#\n")
...
function click_load()
    ...
    if not f then
        ...
        if not f then
            status_label:set_text("No config found at " .. conf_path)
            dlg:update()
            return
        end
    end
    ...
    file_list:clear()
    status_label:set_text("Loaded " .. count .. " schedule(s) from " .. conf_path)
    dlg:update()
end
